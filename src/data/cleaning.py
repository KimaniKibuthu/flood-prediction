...
class DataTransformation:
    ...
    def __init__(self, config: Optional[Dict[str, Any]] = None, data_ingestion: Optional[DataIngestion] = None):
        """
        ...
        self.data = data_ingestion.load_data() if data_ingestion else DataIngestion().load_data()
        logger.info("DataTransformation initialized with data from %s", self.config["data"]["raw_data_path"])
    ...
    def fill_null_values(self, fill_value: Any, column_to_fill: str) -> pd.DataFrame:
        ...
        """
        logger.info("Filling null values in column '%s' with value '%s'", column_to_fill, fill_value)
    ...
    def save_transformed_data(self) -> None:
        ...
        """
        logger.info("Saving transformed data to %s", self.config["data"]["processed_data_path"])
        self.data.to_csv(self.config["data"]["processed_data_path"], index=False)
        logger.info("Transformed data saved to %s", self.config["data"]["processed_data_path"])
...
class DataTransformationPipeline(DataTransformation):
    ...
    def run_pipeline(self) -> None:
        """
        Run all data transformation methods defined in DataTransformation.
        """
        transformations = [
            method_name for method_name in dir(DataTransformation) if callable(getattr(DataTransformation, method_name))
        ]
        for transformation_name in transformations:
            if transformation_name.startswith("__"):
                continue  # Skip internal methods
            transformation_method = getattr(self, transformation_name)
