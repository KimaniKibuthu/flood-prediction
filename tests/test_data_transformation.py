...
from src.cleaning import DataTransformation
...
class MockDataIngestion:
    def load_data(self) -> pd.DataFrame:
        return pd.DataFrame({"old_column1": [1, 2, None], "old_column2": [3, None, 5]})
...
def data_transformation(monkeypatch) -> DataTransformation:
    ...
    return DataTransformation(
        config=mock_load_config(), data_ingestion=MockDataIngestion()
    )
...
def test_save_transformed_data(
    mock_to_csv: MagicMock, data_transformation: DataTransformation
) -> None:
