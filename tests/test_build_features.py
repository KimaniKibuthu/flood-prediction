...
from src.data.build_features import FeatureEngineering, DataProcessor, DataSplitter, FeatureEngineeringPipeline
...
def test_get_spatial_features(mock_data_processor, mock_data):
    ...
    assert all(mock_data_processor.data["Rain_Longitude"] == [10, 40, 90, 160, 250, 360])

def test_split_data(mock_data_processor, mock_data):
    mock_data_processor.data = mock_data
    data_splitter = DataSplitter(mock_data_processor)
    train, test = data_splitter.split_data(mock_data, random_state=42, test_size=0.2, stratify=mock_data["Flood"], save=False)
    assert len(train) == 4
    assert len(test) == 2
